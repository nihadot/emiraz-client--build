...
const PropertyDropDown = ({
  ...
  const handleCountryClick = (country) => {
    ...
    setSelectedCountry({ projectTitle: country.projectTitle, id: country._id }); // Update local state
    setIsOpen(false); // Close dropdown
    onChange({ projectTitle: country.projectTitle, id: country._id }); // Update Formik state
  };
  ...
  return (
    <div className={`flex mx-3 mt-6 flex-col ${mt6 ? 'mt-4' : 'mb-6'}`}>
      <label htmlFor={name} className="sf-medium mt-3 mb-2 font-medium text-sm text-[#000000]">
        Select Projects
      </label>
      ...
      <AnimatePresence>
        {isOpen && options?.length >0 && (
          <motion.div
            ...
            className="border border-[#E4E4E4] mt-03 rounded-[10px] bg-white mt-2 max-h-40 overflow-y-auto shadow-md"
          >
            { options.map((country) => (
              <div
                key={country._id}
                onClick={() => handleCountryClick(country)}
                className={`py-3 px-5 text-sm capitalize text-[#333333] hover:bg-black hover:text-white transition-all duration-300 cursor-pointer ${

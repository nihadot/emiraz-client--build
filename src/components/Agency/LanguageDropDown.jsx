...
const LanguageDropDown = ({
  ...
  const handleClick = (item) => {
    ...
    setSelectedItem({ languageName: item.languageName, id: item._id }); // Update local state
    setIsOpen(false); // Close dropdown
    onChange({ languageName: item.languageName, id: item._id }); // Update Formik state
  };

  useEffect(() => {
    setSelectedItem(value); // Sync value with Formik
  }, [value]);
  ...
  return (
    <div className={`flex mx-3 mt-6 flex-col ${mt6 ? 'mt-4' : 'mb-6'}`}>
      <label htmlFor={name} className="sf-medium font-medium text-sm text-[#000000]">
        Select Language <span className="text-lg text-red-600">*</span>
      </label>

...
const [cities,setCities] = useState([]);
  ...
  useEffect(() => {
    const existingCityIds = existingData?.map((city) => city._id) || [];
    const filteredCities = options?.filter(
      (city) => !existingCityIds.includes(city._id) && !selectedCities.some((selected) => selected._id === city._id)
    );
    setCities(filteredCities);
  }, [options, existingData, selectedCities]);
